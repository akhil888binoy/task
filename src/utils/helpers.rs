...
use get_token::{Token, WRAPPED_TOKENS_MAP};
use types::{
...
pub const BN_ONE: U256 = U256::from(1);
...
pub fn get_by_key<T: Clone>(obj: &HashMap<String, T>, key: &str) -> Option<T> {
    obj.get(key).cloned()
}
...
pub fn get_contract_market_prices(
    ...
    let index_token = get_by_key(tokens_data, &market.index_token_address.to_string())?;
    let long_token = get_by_key(tokens_data, &market.long_token_address.to_string())?;
    let short_token = get_by_key(tokens_data, &market.short_token_address.to_string())?;

    Some(ContractMarketPrices {
        index_token_price: convert_to_contract_token_prices(&index_token.prices, index_token.decimals),
        long_token_price: convert_to_contract_token_prices(&long_token.prices, long_token.decimals),
        short_token_price: convert_to_contract_token_prices(&short_token.prices, short_token.decimals),
    })
}
...
pub fn convert_to_contract_price(price: U256, token_decimals: u32, price_decimals: u32) -> U256 {
...
pub fn expand_decimals(n: U256, decimals: u32) -> U256 {
    n.checked_mul(U256::from(10).unwrap().pow(U256::from(decimals)))
        .unwrap_or(U256::MAX) // Fallback to prevent panic in production
}
...
pub fn get_contract_market_price_for_dynamic_market(
    ...
    for index_token_address in &market.index_token_address {
        if let Some(token_data) = get_by_key(tokens_data, &index_token_address.to_string()) {
    ...
    let index_token_prices = index_tokens
        .iter()
        .map(|token| convert_to_contract_token_prices(&token.prices, token.decimals))
        .collect();
